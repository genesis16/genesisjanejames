...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'genesis_jane_james_woocommerce_breakpoint' );
/**
 ...
 */
function genesis_jane_james_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_products_per_page', 'genesis_jane_james_default_products_per_page' );
/**
 ...
 */
function genesis_jane_james_default_products_per_page() {

	return 8;

}

add_filter( 'woocommerce_pagination_args', 'genesis_jane_james_woocommerce_pagination' );
/**
 ...
 */
function genesis_jane_james_woocommerce_pagination( $args ) {

	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'genesis-jane-james' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'genesis-jane-james' ) );

	return $args;

}

add_action( 'after_switch_theme', 'genesis_jane_james_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
 * Defines WooCommerce image sizes on theme activation.
 *
 * @since 2.3.0
 */
function genesis_jane_james_woocommerce_image_dimensions_after_theme_setup() {
	...
	genesis_jane_james_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'genesis_jane_james_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 ...
 */
function genesis_jane_james_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 2.3.0
 */
function genesis_jane_james_update_woocommerce_image_dimensions() {
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'genesis_jane_james_gallery_image_thumbnail' );
/**
 ...
 */
function genesis_jane_james_gallery_image_thumbnail( $size ) {
