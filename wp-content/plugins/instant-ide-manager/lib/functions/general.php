...
function instant_ide_manager_iide_root_finder() {
	...
	$real_home_path = substr( WP_CONTENT_DIR, 0, strrpos( WP_CONTENT_DIR, '/' ) ) . '/';
	$dirs = glob( $real_home_path . '*' , GLOB_ONLYDIR );
...
function instant_ide_manager_backup_files() {
	...
	foreach( $iide_file_paths as $file => $path ) {
		...
		if ( is_dir( IIDEM_IIDE_DIR . '/' . $path . $file ) )
			instant_ide_manager_copy_dir( IIDEM_IIDE_DIR . '/' . $path . $file, $iide_file_backup_path . '/' . $file );
		elseif ( file_exists( IIDEM_IIDE_DIR . '/' . $path . $file ) )
...
function instant_ide_manager_restore_files() {
	...
	foreach( $iide_file_paths as $file => $path ) {
		...
		if ( is_dir( IIDEM_IDE_DIR . '/' . $file ) ) {
			
			instant_ide_manager_copy_dir( IIDEM_IDE_DIR . '/' . $file, $iide_file_restore_path . '/' . $path . $file );
			instant_ide_manager_delete_dir( IIDEM_IDE_DIR . '/' . $file );
			
		} elseif ( file_exists( IIDEM_IDE_DIR . '/' . $file ) ) {
