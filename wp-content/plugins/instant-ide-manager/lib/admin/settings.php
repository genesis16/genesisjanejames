...
function instant_ide_manager_settings() {
	...
	$real_home_url = substr( WP_CONTENT_URL, 0, strrpos( WP_CONTENT_URL, '/' ) ) . '/';
	$iide_site_url = is_multisite() ? network_site_url() : $real_home_url;
...
function instant_ide_manager_iide_install() {
	...
	instant_ide_manager_copy_dir( IIDEM_IDE_DIR . '/instant-ide', $real_home_path . $_POST['dir'] );
